...
public class CelestialTableMenu extends AbstractContainerMenu {
		...
		for (int l = 0; l < 3; ++l) {
			if (costs[l] > 0) {
				...
				if (!list.isEmpty()) {
					...
					enchantClue[l] = CEBaseEnchantment.getCache().indexOf(ins.enchantment);
	...
	private void onEnchant(Player player, ItemStack tool, ItemStack fuel, int btn, Level level, BlockPos pos) {
		...
		if (!list.isEmpty()) {
			...
			if (flag) {
				newTool = tool.transmuteCopy(Items.ENCHANTED_BOOK);
				enchantSlots.setItem(0, newTool);
			}

			for (var ins : list) {
				newTool.enchant(ins.enchantment.getHolder(), ins.level);
			}
	...
	private Set<CEBaseEnchantment> getForbiddenSet() {
		return access.evaluate((level, pos) -> {
			Set<CEBaseEnchantment> set = new HashSet<>();
			for (BlockPos offset : CelestialTableBlock.BOOKSHELF_OFFSETS) {
				...
				for (int i = 0; i < ch.getContainerSize(); i++) {
					...
					for (var e : EnchantmentHelper.getEnchantmentsForCrafting(stack).keySet()) {
						var be = LegacyEnchantment.firstOf(e, CEBaseEnchantment.class);
						if (be != null) set.add(be);
					}
				}
			}
			return set;
		}, Set.<CEBaseEnchantment>of());
	}
