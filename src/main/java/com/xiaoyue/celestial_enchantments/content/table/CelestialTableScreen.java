...
public class CelestialTableScreen extends AbstractContainerScreen<CelestialTableMenu> {
   ...
   private void renderBook(GuiGraphics g, int pX, int pY, float pTick) {
      ...
      bookModel.renderToBuffer(g.pose(), vertexconsumer, 15728880, OverlayTexture.NO_OVERLAY, -1);
      g.flush();
      g.pose().popPose();
   ...
   public void render(GuiGraphics g, int mx, int my, float pTick) {
      ...
      for (int slot = 0; slot < 3; ++slot) {
         ...
         int idx = menu.enchantClue[slot];
         CEBaseEnchantment e = idx >= 0 && idx < CEBaseEnchantment.getCache().size() ? CEBaseEnchantment.getCache().get(idx) : null;
         int clue = menu.levelClue[slot];
         if (isHovering(60, 14 + 19 * slot, 108, 17, mx, my) && k > 0) {
            List<Component> list = Lists.newArrayList();
            list.add((Component.translatable("container.enchant.clue", e == null ? "" : Enchantment.getFullname(e.getHolder(), clue))).withStyle(ChatFormatting.WHITE));
            if (e == null) {
