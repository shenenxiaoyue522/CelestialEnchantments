...
public class DeathPact extends DeathEnch {
	...
	private static int radius() {
		return CEModConfig.SERVER.ench.armor.deathPackRadius.get();
	}

	private static double chance() {
		return CEModConfig.SERVER.ench.armor.deathPackChance.get();
	}

	private static int dur() {
		return CEModConfig.SERVER.ench.armor.deathPackDuration.get();
	}
	...
	public void onDeath(LivingEntity entity, int level) {
		...
		if (entity instanceof Player player) {
			...
			for (LivingEntity pl : list) {
				int lv = EnchantmentHelper.getEnchantmentLevel(this.getHolder(), pl);
				if (chance(entity, lv * chance())) {
					pl.kill();
				} else {
					pl.addEffect(EFF0.ins(level), pl);
					pl.addEffect(EFF1.ins(level), pl);
					pl.addEffect(EFF2.ins(level), pl);
					pl.addEffect(EFF3.ins(level), pl);
				}
			}
		}
	}
