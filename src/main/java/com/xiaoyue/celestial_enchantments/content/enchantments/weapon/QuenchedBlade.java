...
public class QuenchedBlade extends AttackEnch {
	...
	private static double atk() {
		return CEModConfig.SERVER.ench.weapon.quenchedBladeDamagePerSecond.get();
	}

	private static int maxFire() {
		return CEModConfig.SERVER.ench.weapon.quenchedBladeMaxFire.get();
	}
	...
	public void onHurtTarget(LivingEntity user, LivingEntity target, DamageData.Offence data, int lv) {
		if (target.isOnFire()) {
			...
			data.addHurtModifier(DamageModifier.multBase(lv * i, damageId()));
			target.clearFire();
		}
	}
