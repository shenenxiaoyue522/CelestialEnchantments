...
public class DimensionExplorer extends ArmorEnch {
	...
	private static int dur() {
		return CEModConfig.SERVER.ench.armor.dimensionExplorerDuration.get();
	}
	...
	public static void onLevelChange(Player player, int lv) {
		player.addEffect(LUCK.ins(lv), player);
		player.addEffect(DMG.ins(lv), player);
		player.addEffect(DIG.ins(lv), player);
	}
