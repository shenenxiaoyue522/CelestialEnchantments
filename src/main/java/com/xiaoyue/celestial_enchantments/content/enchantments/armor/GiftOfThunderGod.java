...
public class GiftOfThunderGod extends DefenceEnch {
	...
	private static double heal() {
		return CEModConfig.SERVER.ench.armor.giftOfThunderGodHeal.get();
	}

	private static int duration() {
		return CEModConfig.SERVER.ench.armor.giftOfThunderGodDuration.get();
	}
	...
	public boolean onAttackedImpl(LivingEntity user, DamageData.Attack data, int lv) {
		if (data.getSource().is(DamageTypeTags.IS_LIGHTNING)) {
			user.heal(lv * (float) heal() * (user.getMaxHealth() - user.getHealth()));
			user.addEffect(MOVE.ins(lv), user);
			user.addEffect(RES.ins(lv), user);
			return true;
		}
		return false;
	}
