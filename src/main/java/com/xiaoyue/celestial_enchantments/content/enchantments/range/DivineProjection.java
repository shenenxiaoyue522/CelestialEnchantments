...
public class DivineProjection extends BowEnch {
  ...
  private static double atk() {
		return CEModConfig.SERVER.ench.ranged.divineProjectionDamage.get();
	}
	...
	public void hurtTarget(Arrow arrow, LivingEntity target, int lv, DamageData.Offence data) {
		...
		if (owner != null) {
			data.addHurtModifier(DamageModifier.multTotal(1 + owner.distanceTo(target) * (float) atk(), damageId()));
		}
	}
