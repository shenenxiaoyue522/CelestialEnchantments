...
public class TheSourceOfSin extends DefenceEnch {
	...
	private static double chance() {
		return CEModConfig.SERVER.ench.armor.theSourceOfSinChance.get();
	}

	private static int radius(){
		return CEModConfig.SERVER.ench.armor.theSourceOfSinRadius.get();
	}
	...
	public void onDamagedFinal(LivingEntity user, DamageData.DefenceMax data, int lv) {
		if (data.getAttacker() != null) {
			if (chance(user, lv * chance())) {
				List<LivingEntity> entities = EntityUtils.getExceptForCentralEntity(data.getAttacker(), radius(), radius());
				for (LivingEntity list : entities) {
					list.setLastHurtByMob(data.getAttacker());
					list.setLastHurtMob(data.getAttacker());
				}
			}
		}
	}
